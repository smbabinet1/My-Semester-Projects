...
minweights=[6,50,40,90,65,55,38,65,20,65,30,25,80,31,35,33,22,35,20,80,17,40,40,65,65,80,45,110,12,10,70,50,25,55,70,30,100,13,88,25,50,80,13,10,50,40,55,20,9,50,80,44,35,40,35,33,25,50,45,65,32,55,45,110,85,50,40,7,35,105,7,4,35,80,70,55,24,120,12,4,11,100,11,3,25,40,40,14,45,75,75,50,10,70,9,8,30,30,50,55]
filteredlist=[]
filteredlist2=[]
...
def getDogSize(size):
    ...
    global filteredlist
    if size == "tiny":
        for i in range(len(minweights)):
            if 10 >= minweights[i]:
                filteredlist.append(dogbreeds[i])
    elif size == "small":
        for i in range(len(minweights)):
            if 11 <= minweights[i] and 25 >= minweights[i]:
                filteredlist.append(dogbreeds[i])
    elif size == "medium":
        for i in range(len(minweights)):
            if 26<= minweights[i] and 60 >= minweights[i]:
                filteredlist.append(dogbreeds[i])
    elif size == "large":
        for i in range(len(minweights)):
            if 60 < minweights[i]:
        ...
        print("""Sorry, we don't have any dogs that size on our list!
Please only use 'tiny', small', 'medium', or 'large' when describing the size.""")
    breed=random.choice(filteredlist)
    print("We would reccomend the breed: " + breed + ".")
    return(filteredlist)
def open_image(url):
    ...
    img.show()
def researchbreed():
    breed=input("Please Enter the Dog Breed You Would Like to Research: ")
    if breed in dogbreeds:
        x = dogbreeds.index(breed)
        temper=temperament[x]
        print("This dog breed's temperament is: " + temper)
        print("Here's a picture of the breed!")
        open_image(image[x])
    else:
        print("This Breed is Not in the List! Would You Like to Search for a Different Breed?")
        z=input("yes or no?")
        if z =="yes":
            researchbreed()
...
def dog_purpose():
    x=0
    purpose = input("What is your reason for wanting this dog?")
    for i in range(len(dogbreeds)):
        if purpose in dogpurpose[i]:
            filteredlist2.append(dogbreeds[i])
            x=1
    if x==0:
        print("Sorry, we couldn't find any matching dog breeds for that purpose.")
        print("Would you like to enter a different purpose?")
        y=input("yes or no?")
        if y== "yes":
            dog_purpose()
    else:
        breed=random.choice(filteredlist2)
        print("We would reccommend the breed: " + breed + ".")
        return(filteredlist2)
def dogBreeds():
    print("Welcome to the Dog Breed App!")
    while True:
        print("""Would you like to:
1. See Dogs of your Ideal Size
2. Research A Specific Dog Breed
3. Find a Dog Breed for a Specific Purpose
4. View List of Dogs
5. Quit""")
        m=input("1, 2, 3, 4, or 5?")
        if m == "1":
            getDogSize(input("What size dog do you want? tiny, small, medium, or large?"))
        elif m=="2":
            researchbreed()
        elif m == "3":
            dog_purpose()
